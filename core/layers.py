...
class Linear(Layer):
    ...
    def __init__(self, input_size, output_size, init_type="xavier", bias=True):
        ...
        self.weights = np.random.uniform(
            -bound, bound, (self.input_size, self.output_size)
        )
...
class LeakyReLULayer(Activation):
    def __init__(self, alpha=0.01):
        super().__init__(
            lambda x: leaky_relu(x, alpha), lambda x: leaky_relu_derivative(x, alpha)
        )
...
class Dropout(Layer):
    ...
    def forward(self, x):
        if self.training and self.dropout_rate > 0:
            # Create dropout mask
            self.mask = (np.random.rand(*x.shape) > self.dropout_rate).astype(
                np.float32
            )
...
class BatchNorm1D(Layer):
    def __init__(
        self, num_features, momentum=0.1, eps=1e-5, affine=True, gamma=1.0, beta=0.0
    ):
    ...
    def backward(self, grad_output):
        ...
        # Gradient w.r.t mean
        grad_mean = -np.sum(grad_norm * std_inv, axis=0) + grad_var * (
            -2.0 * np.sum(x_centered, axis=0) / batch_size
        )
