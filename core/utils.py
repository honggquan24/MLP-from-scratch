...
def _serialize_optimizer(optimizer):
    ...
    if isinstance(optimizer, SGD):
        ...
        return {"type": type(optimizer).__name__}  # Save optimizer class name
...
def _deserialize_optimizer(state):
    ...
    if type_ == "SGD":
        ...
        optimizer = Momentum(
            learning_rate=state["learning_rate"], momentum=state["momentum"]
        )
        ...
        optimizer = Adagrad(
            learning_rate=state["learning_rate"], eps=state.get("eps", 1e-8)
        )
...
def save_checkpoint(model, optimizer, file_path):
    ...
    data = {
        "layers": [
            _serialize_layer(layer) for layer in model.layers
