...
class Momentum:
    ...
    def update(self, params, gradients):
        ...
        # v_t = μ * v_{t-1} - α * g_t
        # θ_{t+1} = θ_t + v_t
        self.velocities[params_id] = (
            self.momentum * self.velocities[params_id] - self.learning_rate * gradients
        )
...
class RMSProp:
    ...
    def update(self, params, gradients):
        ...
        self.cache[params_id] = self.decay * self.cache[params_id] + (
            1 - self.decay
        ) * (gradients**2)
        ...
        params -= (
            self.learning_rate * gradients / (np.sqrt(self.cache[params_id]) + self.eps)
        )
...
class Adam:
    ...
    def update(self, params, gradients):
        ...
        self.m[params_id] = (
            self.beta1 * self.m[params_id] + (1 - self.beta1) * gradients
        )

        # v_t = β₂ * v_{t-1} + (1 - β₂) * (g_t)^2
        self.v[params_id] = self.beta2 * self.v[params_id] + (1 - self.beta2) * (
            gradients**2
        )
